...
class Store {
    ...
    lastSeenProdsGetData = autorun(() => {
        ...
        if (this.lastSeenProds.length > 0) {
            api.getAnyProd({ slugs: this.lastSeenProds })
                .then((data) => {
    ...
    getCollections = () => {
        ...
        api.getAllCollections()
            .then((data) => {
                ...
                const maxInColl = Math.floor(
                    this.bannersData.collections.length / 4
                )
    ...
    addToLike = (toUser) => {
        ...
        if (this.likeContainer.length) {
            api.getAnyProd({ slugs: this.likeContainer })
                .then((data) => {
    ...
    addtoCart = (give) => {
        ...
        if (this.auth) {
            api.updateCart(this.productInCartList)
                .then((ok) => {
                    // console.log("ok", ok);
                })
                .catch((err) => {
        ...
        if (this.cartCount) {
            // if (give) {
            api.getAnyProd({ slugs: Object.keys(this.productInCartList) })
                .then((data) => {
                    const timeCont = {}
                    this.certInCart = 0
                    ...
                    Object.keys(data).forEach((prod) => {
                        timeCont[data[prod].slug] = {
                            ...data[prod],
                            countInCart: this.productInCartList[
                                data[prod].slug
                            ],
                        }
                    })
                    ...
                    if (Object.keys(timeCont).length) {
                        Object.keys(timeCont).forEach((el) => {
                            if (
                                typeof this.productInCartList[el] === 'number'
                            ) {
                                if (!timeCont[el].sale) {
                                    dontSaleProdCount += +this
                        ...
                        if (
                            ...
                            let toSaleProdCount = Math.floor(
                                dontSaleProdCount / 3
                            )
                            // console.log("minProdSlugs :>> ", minProdSlugs);
                            for (
                                let index = 0;
                                index < dontSaleProd.length;
                                index++
                            ) {
                                if (
                                    this.productInCartList[
                                    ...
                                    timeCont[
                                        dontSaleProd[index].slug
                                    ].sale_price = Math.floor(
                                        timeCont[dontSaleProd[index].slug]
                                            .regular_price *
                                            (1 -
                                                toSaleProdCount /
                                                    this.productInCartList[
    ...
    cityCheck = autorun(() => {
        ...
        if (
            ...
            const init = () => {
                ...
                if (geolocation) {
                    api.getCity(geolocation.region + ' ' + geolocation.city)
                        .then((data) => {
                            if (data.length) {
                ...
                if (!data[0].product.length) {
                    if (this.activeFilters.choosePoint.length) {
                        ...
                        if (this.activeFilters.count) {
                            let searchQt = ''
                            this.activeFilters.choosePoint.forEach(
                                (filterName) => {
                ...
                if (Object.keys(data[0].sort[0]).length) {
                        ...
                        if (
                            ...
                            sortObj.names.forEach((sn) => {
                                if (sortObj[sn] !== undefined) {
                                    newMeasure.push({
                                        name: sn,
                                        value: sortObj[sn].sort(function (
                                            a,
                                            b
                                        ) {
                                            return a - b
                    ...
                    if (data[0].cats !== undefined && !this.prodCats.length) {
                        ...
                        data[0].cats[0].cats.forEach((elemMain) => {
                            elemMain.forEach((elem) => {
                                if (cats[elem.slugName] !== undefined) {
                                    ...
                                    if (
                                        cats[elem.slugName].childs === undefined
                                    ) {
                                        cats[elem.slugName].childs = []
                                        cats[elem.slugName].childsNameArr = []
                                    }
                        ...
                        if (data.collData !== undefined) {
                            this.dataColl = data.collData
                            document.title =
                                data.collData[0].name + ' - Queen of Bohemia'
                        }
                ...
                if (filterName !== 'choosePoint' && filterName !== 'premium') {
                    if (filterName !== 'measure') {
                        if (
                            ...
                            this.activeFilters[filterName].forEach(
                                (filterValue) => {
                                    onePointFilter.push({
                        ...
                        if (
                            Object.keys(this.activeFilters[filterName]).length
                        ) {
                            Object.keys(this.activeFilters[filterName]).forEach(
                                (name) => {
                                    onePointFilter.push({
                                        ...
                                        'attributes.value': {
                                            $in: this.activeFilters[filterName][
                                                name
                                            ],
                                        },
                                    })
